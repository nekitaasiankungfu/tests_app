...
class _ColorEmotionalAssociationsWidgetState
  ...
  /// Go back to previous emotion within this stage
  void _goBack() {
    if (_currentEmotionIndex > 0) {
      setState(() {
        ...
        _emotions.remove(emotion['id']);
        _responseTimesMs.remove(emotion['id']);
        _startResponseTimer();
      });
    }
  }
        ...
        Padding(
          ...
          child: Column(
            children: [
              ...
              if (_currentEmotionIndex > 0 || widget.onBack != null)
                Padding(
                  padding: const EdgeInsets.only(bottom: 12),
                  child: OutlinedButton.icon(
                    onPressed: _currentEmotionIndex > 0 ? _goBack : widget.onBack,
                    icon: const Icon(Icons.arrow_back, size: 18),
                    label: Text(
                      _currentEmotionIndex > 0
                          ? (widget.locale == 'en' ? 'Previous emotion' : 'Предыдущая эмоция')
